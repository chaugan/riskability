...
(function () {
    ...
    var CONNECTION_FIELDS = [
        ...
        ["model", "Model name, as /v1/models reports it (Fetch models lists them)", "foundation-sec-8b"]
    ];
    var PIPELINE_FIELDS = [
    ...
    function renderConnection(card) {
        ...
        // The model name is whatever the server calls it, and nobody should
        // have to guess it: Fetch models asks the endpoint (with the URL and
        // the auth typed above, or the stored secret) for /v1/models and
        // offers the answer as a list. Picking one writes the text field, so
        // Save works exactly as before; a server that lists nothing, or a
        // name the server does not list, stays typed by hand.
        var pickWrap = el("div", "rk-ai-field rk-ai-modelpick");
        var pickRow = el("div", "rk-ai-modelpick-row");
        var fetchBtn = el("button", "rk-btn", "Fetch models");
        fetchBtn.type = "button";
        var pick = el("select", "rk-select");
        pick.hidden = true;
        var pickMsg = el("span", "rk-dim");
        pickRow.appendChild(fetchBtn);
        pickRow.appendChild(pick);
        pickWrap.appendChild(pickRow);
        ...
        function offerModels(models) {
            ...
            var current = (form.model.value || "").trim();
            if (!models.length) {
                pick.hidden = true;
                pickMsg.textContent = "The endpoint lists no models; type the name by hand.";
                return;
            }
            var head = el("option", null, "Choose a model (" + models.length + " listed)…");
            head.value = "";
            ...
            models.forEach(function (m) {
                ...
                opt.value = m;
                if (m === current) { opt.selected = true; listed = true; }
                pick.appendChild(opt);
            });
            pick.hidden = false;
            if (!current && models.length === 1) {
                form.model.value = models[0];
                pick.value = models[0];
                pickMsg.textContent = "Only one model is served; it has been filled in. Save to keep it.";
            } else if (current && !listed) {
                pickMsg.textContent = "\u201c" + current + "\u201d is not among the models the endpoint lists. "
                    + "Pick one, or serve the model under that name.";
            } else {
                pickMsg.textContent = "Pick one to fill in the model name, then Save.";
            }
        }
        pick.addEventListener("change", function () {
            if (pick.value) {
                form.model.value = pick.value;
                pickMsg.textContent = "Model name set to " + pick.value + ". Save to keep it.";
            }
        });
        fetchBtn.addEventListener("click", function () {
            fetchBtn.disabled = true;
            pickMsg.textContent = "Asking the endpoint for /v1/models…";
            request("POST", { action: "test_connection", config: formValues(),
                ...
                .catch(function (e) {
                    fetchBtn.disabled = false;
                    pick.hidden = true;
                    pickMsg.textContent = "Could not list models: " + e.message;
                });
        });
        form._offerModels = offerModels;
        // With an endpoint already configured the list is fetched on load, so
        // the drop-down is there when the page opens rather than only after a
        // click; the button remains for after the URL or the auth is edited.
        // Deferred a tick: formValues() reads inputs this function creates
        // further down (auth, username, secret, TLS), which do not exist yet
        // at this line.
        if ((cfg.endpoint_url || "").trim()) {
            pickMsg.textContent = "Asking the endpoint for /v1/models…";
            setTimeout(function () {
                request("POST", { action: "test_connection", config: formValues(),
                                  password: "" })
                    .then(function (r) { offerModels(r.models || []); })
                    .catch(function (e) {
                        pickMsg.textContent = "Could not list models: " + e.message
                            + " Fix the connection and press Fetch models.";
                    });
            }, 0);
        }
        ...
        test.addEventListener("click", function () {
            ...
            request("POST", { action: "test_connection", config: formValues(),
                ...
                .then(function (r) {
                    ...
                    // The same answer fills the picker beside the model field.
                    if (form._offerModels) form._offerModels(r.models || []);
                })
                .catch(function (e) {
                    test.disabled = false;
                    saveMsg.textContent = "Failed: " + e.message;
        ...
        analyze.addEventListener("click", function () {
            ...
            request("POST", { action: "test_completion", config: formValues(),
                ...
                .then(function (r) {
                    ...
                    if (r.ok) {
                        var res = r.result;
                        saveMsg.textContent = "Valid result in " + r.latency_ms + " ms: the model said " +
                            res.model_tier + ", score " + res.model_score +
                            " (" + res.recommended_action + "); the app computes the priority itself. " + res.rationale;
                    } else {
